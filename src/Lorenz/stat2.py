...
from main import derive_fractal_key_lorenz  # Lorenz+Julia KDF
...
def chi_square_first_byte(kdf_func, num_samples=50000):
    ...
    print(f"\n=== Lorenz KDF: Chi-square on first byte over {num_samples} samples ===")
...
def measure_throughput(kdf_func, duration=1.0):
    ...
    rate = iters / elapsed
    print(f"\n=== Lorenz KDF: Throughput over {elapsed:.2f} s ===")
...
if __name__ == "__main__":
    ...
    chi_square_first_byte(derive_fractal_key_lorenz, num_samples=50000)

    # 2) Throughput test
    measure_throughput(derive_fractal_key_lorenz, duration=1.0)
